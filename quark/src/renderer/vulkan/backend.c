...
static constexpr char vertex_shader_path[] = QUARK_SHADER_DIR "/triangle.vert.spv";
static constexpr char fragment_shader_path[] = QUARK_SHADER_DIR "/triangle.frag.spv";
...
static QUARK_B8 create_shader_module_from_file(VulkanContext* context, const char* path, VkShaderModule* out_module);
static QUARK_B8 create_vulkan_graphics_pipeline(VulkanContext* context);
static void destroy_vulkan_graphics_pipeline(VulkanContext* context);
...
QUARK_B8 vk_init_renderer_window(GLFWwindow* window) {
    ...
    if (!create_vulkan_graphics_pipeline(&context)) {
...
QUARK_B8 vk_shutdown_renderer_window() {
    ...
    destroy_vulkan_graphics_pipeline(&context);
...
static QUARK_B8 create_vulkan_graphics_pipeline(VulkanContext* context) {
    ...
    if (!create_shader_module_from_file(context, vertex_shader_path, &vertex_shader_module)) {
        return QUARK_FALSE;
    }

    if (!create_shader_module_from_file(context, fragment_shader_path, &fragment_shader_module)) {
